...
| PROD-023 | `PROJECT_LIST` 导出为真正的二维表而非单元格文本：`PROJECT_LIST` 字段从主表列中整体剔除（`plan_export_columns` 新增 `include` 过滤谓词，主表用 `field_type != "PROJECT_LIST"`，块规划用 `field_type == "PROJECT_LIST"`，两者复用同一套顺序/去重/消歧规则），改为在主表下方按“日期 × 字段”追加独立分块：空行 + 标题行（`{日期} {责任人} · {字段标签}`）+ 表头行（固定“项目/工作内容/完成状态”）+ 逐条目数据行；同一天存在多个来源条目时，各来源的项目条目依提交顺序直接展平为连续数据行，不做 `[N]` 来源编号也不按项目名合并去重（因为合并会在“完成状态”这一新增列上产生冲突，且各条目已各自独立成行）；样式复用既有 `_style_header`/`_style_body`（表头加粗、边框、自动换行），并新增 `_style_block_title`；列宽自适应改为跨主表与全部分块的统一一次性计算（分块的“项目/工作内容/完成状态”3 列复用主表最左侧几个字母列，共享同一份列宽） | Accepted | 用户直接指令（2026-08-08），取代 `PROD-022` 首版单元格分组渲染的导出格式，其余字段类型导出/主表结构不受影响 | `项目`/`工作内容`现在是独立单元格，不再共享旧版“项目:X”固定前缀带来的“天然免疫公式注入”特性，因此两者都改为单独调用 `_defuse_formula()`；`完成状态`来自固定枚举映射（`_PROJECT_TASK_STATUS_LABELS`），非用户自由文本，不需要转义；`export_style.py` 新增的 `ProjectListBlockLayout`/`style_report_sheet(..., project_list_blocks=...)` 未新增自动化的字体/边框断言（沿用本文件此前既有测试覆盖的粒度——样式正确性仍以人工验证为准，测试只覆盖结构/取值/公式防护） |
