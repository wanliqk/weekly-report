...
| PROD-026 | 导出 Excel 基础列 `责任人` 从第 2 列移到最后一列，模板字段列整体前移；`日期` 保持第 1 列不变 | Superseded by `PROD-027` | 用户直接指令（2026-08-08） | `export.py::_ColumnLayout` 新增 `owner_column` 字段记录 `责任人` 的 1-based 列号（始终等于表头总数，即最后一列）；`_plan_column_layout()` 改为先放 `日期`（列 1），逐个展开模板字段（`PROJECT_LIST` 仍展开为“项目/工作内容/进度”3 列），最后追加 `责任人`；`build_export_workbook()` 写值处从硬编码 `row[1] = owner_username` 改为 `row[layout.owner_column - 1] = owner_username`；`责任人` 仍是"日级列"（`PROD-024` 的合并集合成员之一），项目数大于 1 时随 `日期`/其余常规字段一起纵向合并；受影响的既有测试（列顺序断言）已同步更新，行为本身（合并/多来源编号/公式转义等）不变；该列本身已随 `PROD-027` 从导出中整体移除 |
| PROD-027 | 导出 Excel 移除 `责任人` 基础列（`PROD-026` 引入的最后一列），不再于表格中展示该列；`日期` 仍固定为第 1 列，模板字段列位置不变 | Accepted | 用户直接指令（2026-08-11） | `export.py` 移除 `_OWNER_HEADER` 常量、`_ColumnLayout.owner_column` 字段、`_plan_column_layout()` 中追加该列的逻辑，`build_export_workbook()` 不再写入该列的值，签名同时移除不再使用的 `owner_username` 形参（`ExportService.create()` 调用处同步简化）；`ExportService` 仍持有 `user`（`user.display_name` 仍用于导出文件名，未受影响）；`export_style.py` 文档字符串中列举“已合并列”示例的 `责任人` 引用同步移除；受影响的既有测试（列顺序、合并范围、单行/空列表断言）已同步更新，其余行为（合并、多来源 `[N]` 编号、公式转义、样式）不变 |
| PROD-028 | `PROJECT_LIST` 条目（`ProjectListEntry`）从 `project`/`content`/`status` 三字段重设计为 8 字段：`project`（工作项目）、`content`（工作步骤）必填；三态 `status` 枚举整体取消，拆成 `planned_completion_date`（预计完成时间节点）、`actual_completion_date`（实际完成时间）两个自由文本列；新增 `owner`（责任人）、`assistant`（协助人）、`required_resources`（所需资源支持）、`completion_notes`（实际完成情况及解决措施），六者均默认 `""` 可选留空 | Accepted | 用户直接指令（2026-08-11），并用 `AskUserQuestion` 确认三点：①两个时间列用自由文本而非严格 `YYYY-MM-DD` 日期选择器；②新增/拆分出的六个字段新增条目时均可选，只有工作项目/工作步骤必填；③企业微信同步文案的项目分段只保留工作项目/工作步骤两行，六个新字段（含条目自带的 `owner`）不进入同步文本 | `schemas/daily_report.py` 重写 `ProjectListEntry`、移除 `ProjectTaskStatus`；`services/daily_report.py::_project_list_error()` 改用“必填 key 子集 `<=` 全量 key 上限”的校验（而非旧版精确 8/3 key 匹配），允许可选 key 整体省略；`services/export.py` 的 `_PROJECT_LIST_SUB_HEADERS` 扩为 8 项表头，`_ColumnLayout.project_list_column_ranges`（旧 3 元组）简化为 `project_list_start_column: dict[str,int]`（只存起始列，宽度恒为 8），写值循环按新增的 `_PROJECT_LIST_FIELD_ORDER` 遍历并对全部 8 列统一 `_defuse_formula()`（不再有“进度用固定枚举天然安全”的特例）；`services/wecom_mapper.py::_format_project_list_block()` 只保留“项目:X\n工作内容:Y”两行，模块顶部“责任人 ambiguity”文档字符串重写论证（旧版靠“schema 里没有责任人属性”，现在 schema 确实有 `owner` 了，但产品决策仍排除在同步文本外）；`docs/需求理解.md` §4.3、`docs/方案设计.md` §7.2 同步修正（删状态映射表，示例文本前缀改“工作项目:”“工作步骤:”）。前端 `types/daily-report.ts` 的 `ProjectListEntry` 字段用 snake_case 直接对应后端 JSON key（不做 camelCase 转换，与 `TemplateFieldData` 现有约定一致）；`DynamicFieldInput.vue` 把逐字段具名 update 函数收敛为一个 `updateProjectListField(index, field, value)`，模板内联箭头函数负责柯里化 index/field（沿用“具名函数处理 unknown 值 + 模板内联箭头做柯里化”的既有风格）；`template-fields.ts::formatProjectListEntries` 改为按固定顺序拼接非空可选字段，全空则只显示“工作项目：工作步骤”。受影响的后端测试（`test_daily_reports_api.py`/`test_export_service.py`/`test_wecom_mapper.py`）与前端测试（`template-fields.test.ts`/`daily-form.test.ts`/`weekly-report.test.ts`）已同步更新 |
...
| PROD-030 | 重连（含换 `form_id`）保留用户已配置的 `field_mapping`，只在从未连接过时套用首次计算的默认值 | Accepted for V1，**recipient_config 部分已被 `PROD-032` 取代** | 用户报告 `ISS-042` 尾注的既有观察项在真实使用中确认为缺陷并要求修复（`ai-docs/issues.md` `ISS-048`） | `field_mapping` 只按本地模板 `field_key` 路由，与远端表单结构无关，重连没有理由重置；`WeComConnectionService._upsert_profile()` 已存在 profile 的分支（含并发插入回退）不再写入新计算的 `field_mapping`，只更新 `form_id`/`template_id`/`destination_fingerprint`/`question_mapping_json`/`schema_fingerprint`/`is_active`/`version`。原本这条决策也覆盖 `recipient_config`（连接时首次计算后重连保留），但 `ISS-054` 发现这个首次计算的默认值本身不可靠，用户随后明确要求"收件人永远不手动填写、必须用模板配置的审批人"，`recipient_config` 因此改交给 `PROD-032` 单独治理——不再是"首次计算、此后保留"，而是"每次连接都重新解析" |
| PROD-031 | 提交时的 `form_id` 从当次 `fork_items` 按 `ctime` 最大解析，只用于本次 HTTP 调用，不回写 `wecom_sync_profiles.form_id`/`destination_fingerprint` | Accepted for V1 | 真实账号提交被拒（`business_code=-1000888`）定位到连接时保存的 fork 已过期，`ai-docs/issues.md` `ISS-051`（`docs/方案设计.md` §10.2 第三次修订） | `destination_fingerprint`/`form_id` 仍是连接时确定的稳定身份，`(daily_report_day_id, destination_fingerprint)` 唯一约束与整套幂等模型（`PROD-029`）建立于此，轮换的 fork id 不得混入；`fork_items` 为空（无 fork 历史）时退回 `profile.form_id`，`get_form_detail()` 用 `formcol/detail` 读取时仍传 `profile.form_id`（该端点接受任意历史 fork id 查询） |
| PROD-032 | 收件人（`recipient_config`）彻底改为系统托管：用户永远不手动填写 vid，且必须使用企业微信模板配置的审批人 vid | Accepted for V1 | 用户在 `ISS-051`/`ISS-052` 排障后明确提出的两条产品要求（"1. 永远不让用户手动去填vid。2. 系统必须指定模板里配置的审批人vid"），`ai-docs/issues.md` `ISS-054`（`docs/方案设计.md` §5.1 步骤 5、§6.3 修订） | 连接/重连三级回退解析：`entries[0].reportvids`（真实已提交记录，WeCom 自己算好的收件人）> `template_info.appro[].vid`（模板自身配置的审批人，首次连接无提交历史时也可用）> `wecom_vid` 自身兜底（`ISS-042` 遗留，仅在前两者都没有时使用）；`PUT /api/v1/wecom/profile` 不再接受 `recipient_config` 输入，只保留只读展示；每次连接/重连都用当次解析结果整体刷新该列（不再是"仅首次连接写入、此后保留"，`recipient_config` 部分因此从 `PROD-030` 分离出来单独治理） |
