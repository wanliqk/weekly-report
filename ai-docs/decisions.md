...
| PROD-022 | 新增模板字段类型 `PROJECT_LIST`（项目列表），支持一篇日报中登记多个项目各自的工作内容和完成状态；`content_json` 中该字段的值为 `[{"project": str, "content": str, "status": "TODO"\|"DOING"\|"DONE"}]`；完成状态固定三态，不走 `select`/`multiselect` 的模板可配置 `options` 机制；导出按项目分组渲染为“项目:X\n- 内容”文本块（后端使用半角冒号以满足 Ruff `RUF001` 全角标点检测，前端展示仍延续既有全角标点习惯），不显示状态 | Accepted | 用户直接指令（2026-08-08），不修改数据库结构，只扩展字段类型处理逻辑（`fields_json`/`content_json` 均为既有 TEXT/JSON 列） | 复用现有 `field_type` 校验通路（`_normalized_options` 已按“非 select/multiselect 类型不得配置 options”的既有规则自动拒绝该类型的 `options`，无需新增分支）；`DailyFieldValue` 新增 `list[ProjectListEntry]` 联合分支，`parse_daily_content`/`parse_day_archive_snapshot` 的 Pydantic smart-union 已实测能正确按列表元素类型（字符串 vs 对象）区分 `multiselect` 与 `PROJECT_LIST`，无需额外判别字段；导出复用既有 `_single_source_cell`/`_multi_source_cell`/公式注入防护管线，按列表首元素类型分流到项目分组渲染分支 |
