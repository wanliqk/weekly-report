...
| ISS-006 | P0 | RESOLVED | 所有权与业务 API 尚未完整实现 | 阶段 4 认证、阶段 5 模板/设置/日报状态机、阶段 6 导出和阶段 7 周报的 owner 强制过滤均已提交并通过独立审查（阶段 6、7 另有专项安全审查逐条核实所有权隔离）；全部面向普通用户的业务 API（认证、用户、模板、日报、设置、导出、周报）owner 边界均已确认。剩余的 admin 手动整库备份（`BACKUP-01`，阶段 8）是不同性质的管理员运维能力，不属于本条"用户业务 API 所有权"范畴，由 RISK-004 单独跟踪 | 阶段 4/5/6/7（均已交付）；阶段 8 `BACKUP-01` 按 RISK-004 单独跟踪 |
...
| RISK-005 | P2 | MITIGATED | 业务自动化覆盖仍需随模块扩展 | 当前后端 169 项、前端 91 项、真实 sidecar 集成 2 项均通过；阶段 7 已新增自然周校验、跨年周/闰周、同周并发唯一、乐观锁冲突和人工编辑隔离覆盖。全链路 Playwright 仍待阶段 9 | 各阶段按 `task.md` 添加契约、权限、并发、集成和 E2E 覆盖 |
