...
| ISS-018 | P1 | MITIGATED | CR-20260807-01 将日报从“同日唯一、单篇三态归档”改为“同日多条目、日期级汇总归档”，与现有 schema/API/周报/导出直接冲突 | 第二版方案已形成日期容器 1:n 条目、正式快照、日期互斥事务、下游来源和 V1 迁移/受限 downgrade，等待用户确认；当前代码仍是 V1 | 用户确认方案后，阶段 10A～10E 按设计实现并通过旧库迁移、并发、周报/导出和 E2E 才能 RESOLVED |
...
| ISS-020 | P1 | MITIGATED | 首次初始化改为普通用户 + 默认管理员双账号 | 已冻结固定 `admin`、密码分别哈希、双账号/默认数据原子创建、admin 首次登录强制改密和并发互斥；当前 API 仍只建一名 admin | 阶段 10A 实现并通过并发初始化、回滚、哈希和强制改密测试后关闭 |
