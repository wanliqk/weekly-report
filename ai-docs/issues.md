...
| ISS-021 | P1 | MITIGATED | “用户管理支持删除”与 V1 永久保留业务记录、外键 RESTRICT 和无业务删除 API 冲突 | BE-10B 已实现：无业务记录（`daily_report_days`/`weekly_reports`/`export_jobs`）可物理删除并清理默认关联资源，有业务记录返回 `40910`；当前账号自我删除保护、末位有效管理员条件 `DELETE`（已用双管理员并发互删测试验证只保留一个）、确认用户名精确匹配、原因必填均已实现；检查后并发产生业务记录的场景由 `ON DELETE RESTRICT` 捕获为 `IntegrityError` 兜底 | 后端安全删除和外键竞态已通过自动化测试和独立安全审查；FE-10 落地用户管理删除入口、QA-10 完成端到端验收后才能 RESOLVED；级联永久删除仍不在范围 |
| ISS-022 | P2 | MITIGATED | 统计指标需要统一日历口径 | 已冻结自然日完成率、来源条目篇数、`week_start` 周报归属、今天/昨天连续记录和未来月 null 规则；BE-10C 已实现 `GET /statistics/monthly`，当前月/历史月/未来月分母、完成率四舍五入、连续天数规则均有纯函数测试覆盖并与 `docs/方案设计.md` §8.6 示例逐项核对 | 后端查询已完成；FE-10 完成统计页、界面口径说明和跨月/闰日端到端验证后关闭 |
