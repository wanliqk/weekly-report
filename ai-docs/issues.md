...
| ISS-018 | P1 | MITIGATED | CR-20260807-01 将日报从“同日唯一、单篇三态归档”改为“同日多条目、日期级汇总归档”，与 V1 schema/API/周报/导出直接冲突 | BE-10A 交付日期容器、正式单来源快照、V1 数据/周报 JSON 迁移和受限 downgrade；BE-10B 已实现真正的同日多篇创建（`client_request_id` 幂等）、草稿删除、日期级归档事务（聚合全部已提交条目、日期行并发互斥）并移除 V1 单篇兼容桥；BE-10C 已把周报生成/重生成和导出改为读取 `daily_report_days.archive_snapshot_json` 的日期级正式快照（多来源条目正确聚合为一份正式记录），移除条目级查询；FE-10 已把“我的日报”月历、日期级归档按钮/正式快照展示、周报多来源卡片全部切换为日期级 UI | 后端与 Electron/Vue 界面均已完成并经真实冒烟验证；`QA-10` 完成端到端专项验收和 E2E 覆盖后才能 RESOLVED |
| ISS-019 | P1 | MITIGATED | admin 撤销其他用户提交改变了现有“admin 不查看/操作他人业务数据”的权限边界 | BE-10B 已实现：`list_submitted_awaiting_archive`/`get_submitted_metadata` 均为原始列选择，SQL 层面不选择 `content_json`/`template_snapshot_json`；撤销要求 `{version,reason}`，原因非空校验；成功后写入 `admin_audit_events`（仅白名单元数据，`metadata_json` 只含 `work_date`）；所有者一侧可见最近撤销原因和时间。独立安全审查确认无正文/模板快照泄露路径；FE-10 已交付 `/admin/daily-reports` 管理界面（待归档列表+撤销弹窗+审计记录），前端字段与后端最小元数据类型逐一对应 | 后端权限/审计和 Electron 管理界面均已完成；`QA-10` 完成端到端验收后才能 RESOLVED |
| ISS-020 | P1 | RESOLVED | 首次初始化改为普通用户 + 默认管理员双账号 | BE-10A 已实现固定 `admin`、密码分别 Argon2id 哈希、双账号及双方默认资源原子创建、admin 首次登录强制改密、管理员重置后强制改密；顺序/并发重复初始化、事务回滚、哈希不同和业务 API 拦截均有自动化测试；FE-10 已交付 `/change-password` 强制改密路由与拦截 | 阶段 10A 已完成并通过后端 199 项全量测试；FE-10 的强制改密界面已通过真实 Electron 冒烟验证 |
| ISS-021 | P1 | MITIGATED | “用户管理支持删除”与 V1 永久保留业务记录、外键 RESTRICT 和无业务删除 API 冲突 | BE-10B 已实现：无业务记录（`daily_report_days`/`weekly_reports`/`export_jobs`）可物理删除并清理默认关联资源，有业务记录返回 `40910`；当前账号自我删除保护、末位有效管理员条件 `DELETE`（已用双管理员并发互删测试验证只保留一个）、确认用户名精确匹配、原因必填均已实现；检查后并发产生业务记录的场景由 `ON DELETE RESTRICT` 捕获为 `IntegrityError` 兜底；FE-10 已在“用户管理”页新增删除入口，按 `can_delete`/`cannot_delete_reason` 提前禁用 | 后端安全删除、外键竞态和 Electron 删除入口均已完成；`QA-10` 完成端到端验收后才能 RESOLVED；级联永久删除仍不在范围 |
| ISS-022 | P2 | MITIGATED | 统计指标需要统一日历口径 | 已冻结自然日完成率、来源条目篇数、`week_start` 周报归属、今天/昨天连续记录和未来月 null 规则；BE-10C 已实现 `GET /statistics/monthly`，当前月/历史月/未来月分母、完成率四舍五入、连续天数规则均有纯函数测试覆盖并与 `docs/方案设计.md` §8.6 示例逐项核对；FE-10 已交付 `/statistics` 页面（月历、四项指标卡、未来月/无数据显示 `--`/0） | 后端查询和统计页界面均已完成并经真实冒烟验证；`QA-10` 完成跨月/闰日端到端验证后关闭 |
...
| ISS-024 | P2 | OPEN | FE-10 把 Electron/Vue 界面全面改为第二版形状后，既有 `electron/e2e/*.spec.ts`（`primary-path`/`auth-failures`/`daily-validation`/`admin-guard`/`stale-version-conflict`，共 5 个 QA-09 时期建立的 spec）断言仍是 V1 UI，`npm run test:e2e` 现在会失败 | 例如 `helpers/app.ts::DEFAULT_ADMIN.username='admin'` 会命中 BE-10A 新增的保留用户名拒绝（第二版首次初始化的普通用户不得叫 `admin`）；页面标题“日报工作台”已改名“我的日报”；日报详情页的单篇归档按钮已不存在（归档改为日期级，收口到日历页）。FE-10 阶段用 Playwright `_electron` 手写了一次性冒烟脚本验证了双账号初始化→强制改密→日历创建/提交/归档→周报→统计→设置→管理员页面的真实链路全部通过，但未把脚本保留为正式 E2E 用例（避免与 `QA-10` 重复建设） | 非阻塞，不影响 FE-10 交付；`QA-10` 需要重写这 5 个 spec（或替换为等价的 V2 spec）为第二版 UI 断言，之后 `npm run test:e2e` 才能重新作为常规质量门禁 |
