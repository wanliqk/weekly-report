...
| ISS-006 | P0 | MITIGATED | 所有权与业务 API 尚未完整实现 | 阶段 4 认证与用户管理已实现、提交并通过独立审查；阶段 5 正在落地模板、设置、日报状态机和 owner 强制过滤，周报等后续业务 API 仍待各阶段实现 | 阶段 5 每个日报 Repository 查询强制 owner 条件；后续业务模块继续复用该边界，全部核心业务完成后关闭 |
