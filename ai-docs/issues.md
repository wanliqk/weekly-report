...
| ISS-018 | P1 | MITIGATED | CR-20260807-01 将日报从“同日唯一、单篇三态归档”改为“同日多条目、日期级汇总归档”，与 V1 schema/API/周报/导出直接冲突 | BE-10A 交付日期容器、正式单来源快照、V1 数据/周报 JSON 迁移和受限 downgrade；BE-10B 已实现真正的同日多篇创建（`client_request_id` 幂等）、草稿删除、日期级归档事务（聚合全部已提交条目、日期行并发互斥）并移除 V1 单篇兼容桥。周报/导出仍读取条目级 `archived` 状态，尚未切换为日期级正式快照，多条目场景下游聚合语义仍不正确 | 阶段 10C 完成周报/导出/统计的日期级正式来源适配，阶段 FE-10/QA-10 完成 Electron 与 E2E 后才能 RESOLVED |
