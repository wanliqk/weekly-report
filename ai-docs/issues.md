...
| ISS-018 | P1 | OPEN | CR-20260807-01 将日报从“同日唯一、单篇三态归档”改为“同日多条目、日期级汇总归档”，与现有 schema/API/周报/导出直接冲突 | 当前代码仍受 `uq_daily_reports_user_work_date`、`draft/submitted/archived`、提交自动归档和周报直接来源约束，不能通过改页面文案满足；错误实现会造成重复汇总或历史数据丢失 | 完成第二版数据模型、状态/聚合事务、API、周报/导出来源和 V1 历史数据迁移/回滚方案后再排实现 |
| ISS-019 | P1 | OPEN | admin 撤销其他用户提交改变了现有“admin 不查看/操作他人业务数据”的权限边界 | 需求已明确允许撤销，但是否可查看正文及管理员操作入口未确认；权限过宽会破坏隐私，权限过窄可能无法定位条目 | 用户确认正文可见性；方案定义最小查询字段、撤销原因/审计、所有权例外和越权响应 |
| ISS-020 | P1 | OPEN | 首次初始化改为普通用户 + 默认管理员双账号，但默认管理员用户名和首次登录改密策略未确认 | 当前 bootstrap API/事务只创建一名 admin；双账号初始化还需处理用户名冲突、并发互斥和两套默认数据 | 当前假设用户名 `admin`、初始密码相同且分别哈希；用户确认用户名与“提醒/强制改密”后设计新初始化契约 |
| ISS-021 | P1 | OPEN | “用户管理支持删除”与 V1 永久保留业务记录、外键 RESTRICT 和无业务删除 API 冲突 | 直接物理删除有数据用户可能导致历史丢失或复杂级联；只停用又可能不满足用户对“删除”的预期 | 当前安全基线为无业务记录可删、有业务记录只停用；若需级联永久删除，用户需确认范围、备份、二次验证和恢复策略 |
| ISS-022 | P2 | OPEN | 统计指标缺少经用户确认的日历口径 | 已整理自然日完成率、来源条目篇数、`week_start` 周报归属和连续记录算法，但工作日/周末、篇数是否等于完成天数仍可能改变结果 | 用户确认自然日与篇数口径；之后在方案中定义查询契约、边界日期和性能索引 |
