...
> 状态：V1 模块已实现；第二版日报聚合、管理员撤销与统计模块待重划
> 更新日期：2026-08-07

本文件说明目标模块边界。模块被列出不表示对应代码已经存在或完成；实际进度以当前工作树、`progress.md` 和 `task.md` 为准。

> CR-20260807-01 已新增日期级日报聚合、草稿删除、管理员撤销提交、双账号初始化和个人统计。现有 M05/M06/M08/M09/M10/M13/M15/M16 的职责和页面映射需要在方案设计阶段更新；本文件当前矩阵仍用于描述 V1 实现事实。
