...
> 实现状态：数据层、Electron 凭证桥、协议 Client、字段 Mapper、连接/同步编排 Service、公开/Main-only API 与 Renderer 交互（`WECOM-02..07`）均已实现，`wecom_sync=true`。受控企业微信测试账号、生产打包和发布级全链路验收仍属于 `WECOM-08`，具体事实以 `progress.md`/`task.md` 为准。
