...
- 已完成提交：`3a9fdbc`（工程基线）、`7386cae`（Desktop Bootstrap）、`8480515`（数据基础与 API Foundation）、`b6b1b47`（补充编码规则）、`1a50e75`（认证与用户管理）、`00e647f`（关闭阶段 4 并启动阶段 5 的状态文档）、`1d965fe`（模板、设置与日报闭环）、`d6ab86e`（查询导出与桌面保存）、`fd4df17`（关闭阶段 6 并回填提交号）、`346b0ea`（周报闭环）、`2584133`（关闭阶段 7 并回填提交号）、`00caa33`（设置能力与受控备份）、`0148171`（质量与发布）。
- 当前所在阶段：V1 阶段 9 已完成；CR-20260807-01 第二版增量处于需求确认阶段，`REQ-10=REVIEW`，尚未进入方案设计或代码实现。
...
- 当前阻塞：V1 无业务/技术阻塞；第二版在方案设计前需确认默认管理员策略、管理员正文权限、用户删除语义和统计口径（ISS-018..ISS-022）。
- 第二版需求整理事实：已在 `docs/需求理解.md` 追加同日多条目、草稿删除、管理员撤销、日期级汇总归档、菜单改名、双账号初始化和个人统计的完整增量需求及验收标准；`ai-docs/requirements.md` 已同步覆盖规则。当前代码、数据库和 API 仍是 V1 行为，未做任何业务实现变更。
...
- CR-20260807-01 全部第二版能力：同日多日报条目、草稿物理删除、admin 撤销提交、日期级聚合归档、双账号首次初始化、用户安全删除、菜单改名和个人统计。
- 第二版数据/API/权限/迁移方案，尤其是 `uq_daily_reports_user_work_date` 替换、正式日报与来源关系、撤销审计、旧库升级和周报/导出来源切换。
...
1. 用户确认默认管理员用户名与首次登录改密策略。
2. 用户确认 admin 撤销提交时能否查看他人正文。
3. 用户确认有业务数据用户只停用还是允许级联永久删除。
4. 用户确认完成率按自然日、日报篇数按来源条目统计的口径。
5. 完成 `DESIGN-10`，统一更新方案、数据库、API、迁移和验收后，才可启动 `BE-10`/`FE-10`。
