...
- 已完成提交：`3a9fdbc`（工程基线）、`7386cae`（Desktop Bootstrap）、`8480515`（数据基础与 API Foundation）、`b6b1b47`（补充编码规则）、`1a50e75`（认证与用户管理）、`00e647f`（关闭阶段 4 并启动阶段 5 的状态文档）、`1d965fe`（模板、设置与日报闭环）、`d6ab86e`（查询导出与桌面保存）、`fd4df17`（关闭阶段 6 并回填提交号）、`346b0ea`（周报闭环）、`2584133`（关闭阶段 7 并回填提交号）、`00caa33`（设置能力与受控备份）、`0148171`（质量与发布）、`a866872`（第二版增量需求）。
- 当前所在阶段：V1 阶段 9 已完成；CR-20260807-01 的 `REQ-10=DONE`、`DESIGN-10=REVIEW`，第二版尚未编码，等待用户确认技术方案。
...
- 当前阻塞：无技术阻塞；`DESIGN-10` 按流程等待用户确认，确认前不启动实现。ISS-018..ISS-022 已有明确方案并降为 `MITIGATED`，仍需实现和测试后才能关闭。
- 第二版需求与设计事实：增量需求、推荐口径和验收已确认；已完成日期容器 1:n 来源条目、日期级正式快照/归档事务、管理员最小元数据撤销与审计、双账号/强制改密、安全删除、自然日统计、V1 数据迁移/受限 downgrade、周报/导出适配和 Electron 路由设计，等待用户确认。当前代码、数据库和 API 仍是 V1 行为。
...
- CR-20260807-01 全部第二版代码能力：同日多日报条目、草稿物理删除、admin 撤销提交、日期级聚合归档、双账号首次初始化、用户安全删除、菜单改名和个人统计。
- 第二版数据/API/权限/迁移方案已经完成文档设计，但 Alembic、Model、Service、Repository、API、Vue 页面和自动化测试均尚未实现。
...
第二版的下一检查点是用户确认 `DESIGN-10`；确认后进入 `BE-10A`：

1. 新增日期容器、管理员审计和 `must_change_password` 的 ORM/Alembic 契约。
2. 用当前 V1 schema 构造旧库副本，验证升级映射、行数/外键/快照等价和受限 downgrade。
3. 实现普通用户 + 固定 admin 的原子 bootstrap、分别哈希和强制改密依赖。
4. 重建设置表移除自动归档字段，并通过 Ruff、mypy、pytest 与 `git diff --check`。
5. 10A 独立提交完成前，不开始日报聚合事务或 Electron 页面。
