...
- 当前所在阶段：CR-20260807-01 的 `REQ-10`、`DESIGN-10`、`BE-10A`、`BE-10B`、`BE-10C`、`FE-10` 已完成；当前可进入 `QA-10`。
...
- 当前阻塞：无技术阻塞；ISS-020 已随 BE-10A 后端实现关闭；ISS-018/019/021/022 的后端与 Electron/Vue 界面均已随 BE-10B/BE-10C/FE-10 完成，仍维持 MITIGATED（未升级 RESOLVED）——按 `issues.md` 的关闭条件，需等 `QA-10` 补齐端到端验收和真实 E2E 覆盖后才能转 RESOLVED；ISS-023（BE-10B 契约偏差）已 RESOLVED；新记录 ISS-024（FE-10 UI 改版导致既有 `electron/e2e/*.spec.ts` 断言过期，`npm run test:e2e` 现在会失败），非阻塞，属 `QA-10` 既定范围。
...
- `QA-10`：第二版旧库迁移、同日并发、汇总原子性、权限隔离、删除保护、跨月/闰日统计的专项验收，以及重写 `electron/e2e/*.spec.ts` 为 V2 UI 形状（现有五个 spec 断言 V1 页面文案/流程，运行会失败，见 `issues.md` ISS-024）。
- 第二版迁移与认证基线（BE-10A）、日报聚合/管理员撤销/用户安全删除后端（BE-10B）、周报/导出/统计适配后端（BE-10C）、全部 Electron/Vue 界面（FE-10）均已完成；仅 `QA-10` 的端到端专项验收和 E2E 套件重建尚未开始。
...
1. 用当前 V1 schema 的真实测试夹具构造旧库副本，验证升级到第二版迁移（`8b1d4e6f2a90`）后的行数、外键、正文/模板快照逐字节等价。
2. 同日多篇创建、日期归档、管理员撤销的并发/权限专项验收（在真实 Electron+FastAPI 环境而不仅是后端单元测试层面复核）。
3. 跨月/闰日统计口径的端到端验证（不仅是后端纯函数测试）。
4. 重写 `electron/e2e/*.spec.ts`（五个既有 spec 仍是 V1 UI 断言，需要改为覆盖双账号初始化、强制改密、日历创建/归档、管理员撤销、统计等 V2 流程，见 `issues.md` ISS-024）。
5. QA-10 独立提交完成后，第二版 CR-20260807-01 全部交付完毕。

阶段 10B（日报聚合、管理员撤销与用户安全删除）已实现、自测、质量门禁与独立安全审查通过，实现提交 `784f96c`（见上方"第二版日报聚合、管理员撤销与用户安全删除（阶段 10B，BE-10B）"一节），并随发现的契约偏差以修正提交 `c7ed342` 补齐（ISS-023）。阶段 10C（周报、导出与统计适配）已实现、自测、质量门禁与独立安全审查通过，实现提交 `6255755`（见上方"第二版周报、导出与统计适配（阶段 10C，BE-10C）"一节）。阶段 10D（Electron/Vue 界面，`FE-10`）已实现、自测、质量门禁与真实 Electron 冒烟验证通过，实现提交待创建（见上方"第二版 Electron/Vue 界面（阶段 10D，FE-10）"一节）。
