...
- 2026-08-11：应用户直接指令移除导出 Excel 的 `责任人` 基础列（记为 `PROD-027`，取代未提交的 `PROD-026`），表格不再展示该列，`日期` 仍固定第 1 列，模板字段列位置不变。`export.py` 移除 `_OWNER_HEADER` 常量、`_ColumnLayout.owner_column` 字段及其填充逻辑，`build_export_workbook()` 签名同步移除不再使用的 `owner_username` 形参；`ExportService.create()` 调用处相应简化，`user` 仍保留（`user.display_name` 继续用于导出文件名）；`export_style.py` 文档字符串中的 `责任人` 示例引用一并移除。测试：`test_export_service.py`（移除 `_OWNER_HEADER` 常量，更新列表头/行值/合并范围断言，含单行、空列表、多来源、多项目合并等既有用例）与 `test_exports_api.py`（移除该列的表头/取值断言）已同步更新，未新增用例（既有维度整体移除，非新增行为）。实际门禁：`uv run ruff check .`（除本次未触碰的 `export.py::_export_file_name` 一处既有、与本次改动无关的 E501 漂移外通过）、`uv run mypy`（涉及文件 strict 通过）、`uv run pytest`（导出相关两个测试文件全部通过；全量回归见提交前最终核对）均已执行。未改动前端。未创建提交，等待用户确认。
- 2026-08-11：应用户直接指令把 `PROJECT_LIST` 条目（`ProjectListEntry`）从 `project`/`content`/`status` 三字段重设计为 8 字段（记为 `PROD-028`）：项目→工作项目、工作内容→工作步骤（均保持必填）；三态 `status` 枚举整体取消，拆成预计完成时间节点/实际完成时间两个自由文本列；新增责任人/协助人/所需资源支持/实际完成情况及解决措施，六个新增/拆分字段均可选。落地前用 `AskUserQuestion` 确认了三点会显著改变实现形状的细节：两个时间列用自由文本而非日期选择器；六个新字段新增条目时可选（不像旧版 project/content/status 三者皆必填）；企业微信同步文案的项目分段只保留工作项目/工作步骤两行，新增字段（含条目自带的 `owner`）不进入同步文本——这一点直接推翻了 `wecom_mapper.py` 旧文档字符串"schema 里没有责任人属性所以责任人只能是独立自定义字段"的论证前提（现在 schema 确实有 `owner` 了），已重写为"新字段仅内部使用，产品决策排除在同步文本外"。

  这是一次跨 5 个消费点的数据模型改动，不只是导出表头文案调整：`schemas/daily_report.py` 重写 `ProjectListEntry`（`project`/`content` 必填，其余默认 `""`）、移除 `ProjectTaskStatus`；`services/daily_report.py::_project_list_error()` 从"精确 key 集合匹配"改为"必填 key 子集 `<=` 全量 key 上限"，允许可选 key 整体省略也允许显式传空串；`services/export.py` 的 `_PROJECT_LIST_SUB_HEADERS` 扩为 8 项表头，`_ColumnLayout.project_list_column_ranges`（旧 3 元组）简化为 `project_list_start_column: dict[str,int]`（只存起始列，宽度恒为 8），写值循环改为按新增的 `_PROJECT_LIST_FIELD_ORDER` 遍历、对全部 8 列统一 `_defuse_formula()`（比旧版更简单，不再有"进度用固定枚举天然安全"的特例）；`services/wecom_mapper.py::_format_project_list_block()` 只保留"项目:X\n工作内容:Y"两行。`docs/需求理解.md` §4.3、`docs/方案设计.md` §7.2 同步修正（删状态映射表和三态转换描述，示例文本前缀改"工作项目:""工作步骤:"）。前端 `types/daily-report.ts` 的 `ProjectListEntry` 字段用 snake_case 直接对应后端 JSON key（沿用 `TemplateFieldData` 现有约定，不做 camelCase 转换，此前起草时曾误用 camelCase 已自行发现改正）；`DynamicFieldInput.vue` 把逐字段具名 update 函数收敛为一个 `updateProjectListField(index, field, value)`，模板内联未类型化箭头函数负责柯里化 index/field（沿用"具名函数处理 unknown 值 + 模板内联箭头做柯里化"的既有风格，只是避免为 8 个字段各写一个近乎重复的具名函数），表单布局从单行 4 列 grid 改为"三列一行 ×2 + 两个整行 textarea"分组卡片；`template-fields.ts::formatProjectListEntries` 改为按固定顺序拼接非空可选字段，全空则只显示"工作项目：工作步骤"。

  受影响测试：后端 `test_daily_reports_api.py`（round-trip、畸形条目参数化列表改为"未知 key/缺字段/多余 key/类型错误"等新场景）、`test_export_service.py`（表头、行值、合并范围、公式转义断言全部按 8 列重写，公式转义测试顺带扩展覆盖新增的 `owner` 列）、`test_wecom_mapper.py`（项目分段格式断言去掉"完成状态"行，重写"责任人 ambiguity"相关测试的文档字符串）；前端 `template-fields.test.ts`/`daily-form.test.ts`/`weekly-report.test.ts` 的内联字面量与断言字符串同步新形状。实际门禁：后端 `uv run ruff check .`（除既有、与本次无关的 `export.py::_export_file_name` E501 漂移外通过）、`uv run mypy app tests`（strict，146 源文件，通过；仓库根 `wx-ribao/` 遗留脚本的既有 31 项 mypy 错误与本次改动无关，未改动该文件）、`uv run pytest`（JUnit XML 确认 **499 项、0 错误、1 失败（`test_config.py` 本机 `.env` 内 `WEEKLY_REPORT_RUNTIME_SECRET=` 导致的既有环境问题，与本次改动无关，单独执行该用例前后均复现同一失败）、0 跳过**）均实际执行；前端 `npm run lint`（0 error/0 warning）、`npm run typecheck`、`npm test`（**27 文件 261 项全部通过**）、`npm run build`、`npx prettier --check`（改动文件）均实际执行并通过。尚未在真实 Electron/浏览器中手动验证新表单 UI 与导出结果（工具限制：浏览器自动化只能驱动 Chrome 标签页，无法驱动 Electron 窗口；用户确认跳过人工验证，直接以自动化门禁结果收尾）。已随提交 `c631505` 交付（该提交同时包含未单独提交的 `PROD-027`）。
- 2026-08-11：应用户直接指令为日报模板字段新增“是否显示在 Excel 导出中”开关 `show_in_export`（记为 `PROD-029`），bool 类型，默认 `true`。`schemas/template.py` 的 `TemplateFieldInput`/`TemplateFieldData` 均新增该字段并给默认值（`TemplateFieldData` 一侧的默认值确保本次改动前已归档、不含该 key 的历史快照 JSON 仍能正常反序列化，不需要迁移）；`services/template.py::build_next_template_fields()` 透传；`services/user.py::default_template_fields()` 的两个默认字段显式写 `True`（保持该字典一贯的“不依赖隐式默认”风格）。核心逻辑在 `services/export.py::plan_export_columns()`：新增 `show_by_key` 并复用既有“最近快照胜出”合并规则（与 `label`/`field_type` 完全一致）——字段某天可见、但更晚归档的快照已关闭显示时，整个字段从导出结果中消失（不是按天分别决定，是按 `field_key` 全局决定）；同名消歧用的 `header_counts` 同步改为只统计过滤后仍可见的字段，避免一个被隐藏字段和一个可见字段偶然同名时，给可见字段加上不必要的 `field_key` 后缀（原实现是先统计全部字段的标签频次、再过滤，会漏掉这个边界情况，属于设计阶段发现并修正、非事后 bug）。前端 `types/template.ts`、`utils/template-fields.ts`（`TemplateFieldDraft` 及三个转换函数）同步新增；`TemplatesView.vue` 在既有“启用/必填” `el-switch` 旁加第三个“导出显示/导出隐藏”开关，复用现有 flex 布局无需改样式。测试：后端新增 `test_show_in_export_defaults_to_true_and_round_trips_when_toggled_off`（`test_templates_api.py`）、`test_create_excludes_a_field_marked_hidden_from_export` 与 `test_create_applies_most_recent_snapshot_wins_to_show_in_export`（`test_export_service.py`，后者特意镜像既有“最近标签胜出”测试的写法以验证合并规则的一致性）；前端 `template-fields.test.ts` 新增默认值/透传断言，`daily-form.test.ts`/`wecom.test.ts` 的字段 fixture 补齐该新必填属性。实际门禁：后端 `uv run ruff check .`/`ruff format --diff`（除既有、与本次无关的 `export.py::_export_file_name` E501 漂移外通过）、`uv run mypy`（涉及文件 strict 通过）、`uv run pytest`（JUnit XML 确认 **502 项、0 错误、1 失败（同前一条记录的既有本机 `.env` 环境问题，与本次改动无关）、0 跳过**）均实际执行；前端 `npm run lint`（0 error/0 warning）、`npm run typecheck`、`npm test`（**27 文件 262 项全部通过**）、`npm run build`、`npx prettier --check`（改动文件）均实际执行并通过。未做真实 Electron/浏览器人工 UI 验证（同前一条记录的工具限制）。未创建提交，等待用户确认。
...
## 30. 收件人彻底改为系统托管，取消手动填 vid（`ISS-054`/`PROD-032`）

- 用户没有等确认 -1000888 是否解决，而是直接提出两条更根本的产品要求："1. 永远不让用户手动去填vid。2. 系统必须指定模板里配置的审批人vid"。这比 `ISS-052` 的范围更大：`ISS-052` 只是把默认值从"提交者自己"改成"`reportvids`"，但（a）设置页仍然留着可以手动改的多选框，（b）`reportvids` 依赖 `entries[0]`，真正第一次连接、还没有任何提交历史的用户拿不到它，那种情况下逻辑仍会退回到已经证明不可靠的自身 vid 兜底。
- 新增 `WeComTemplateApprover(vid, name)` 和 `WeComTemplateInfo.appro`，解析 `body.template_info.appro[]`——这是模板自身配置的审批人名单，模板级、不依赖任何提交历史，因此哪怕是全新连接也一直可用；解析防御式处理（缺失/单条损坏直接跳过，不阻断连接），跟既有 `fork_items` 解析同一套原则。收件人解析改为三级：`entries[0].reportvids`（已提交记录里 WeCom 自己算好的，最贴近真实场景）> `template_info.appro[].vid`（模板配置的审批人，首次连接也可用）> `wecom_vid` 自身兜底（`ISS-042` 遗留，只在前两者都拿不到时用）。
- `PUT /api/v1/wecom/profile` 的 `WeComProfileUpdateRequest` 直接移除 `recipient_config` 字段（`field_mapping` 从可选改为必填，变成唯一能更新的东西）；`WeComConnectionService.update_profile()` 和 `WeComSyncProfileRepository.update_config_if_version()`（改名 `update_field_mapping_if_version`）同步简化，不再有"传 `None` 就保留旧值"的分支。`_upsert_profile()` 里原本"已存在 profile 就保留 `recipient_config`"（`ISS-048`/`PROD-030` 的一部分）整体去掉：现在无论是不是第一次连接，`recipient_config` 都用当次解析结果整体覆盖；`field_mapping` 保留不变的行为不受影响，`PROD-030` 收窄为只管 `field_mapping`，`recipient_config` 分给新的 `PROD-032` 单独治理。
- `WeComSettingsCard.vue` 删掉"管理者接收人 vid（mngreporter）"和"填报接收人 vid（reporter）"两个 `allow-create` 多选输入框（连带原来提示"vid 无法在本应用内查询,请手动输入"的说明文字），改成一行只读文案："当前收件人 vid：xxx、yyy（系统自动从企业微信表单识别，无需手动填写；重新连接后会自动刷新）"。`saveMapping()` 现在只提交 `field_mapping`，不再校验"至少填一个收件人 vid"（这个校验本身也不再有意义，收件人不再是这个表单要管的事）。
- 同步修订 `docs/需求理解.md`（§2 场景描述）、`docs/方案设计.md`（§5.1 步骤 5、§6.3 `recipient_config_json`、§9.1 API 表）、`ai-docs/api.md`。
- 新增 4 项后端 pytest：`appro` 解析成功、`appro` 单条畸形条目被跳过不影响其余、`update_profile` 只改字段映射且 `recipient_config`/`form_id` 不受影响、重连后字段映射保留但收件人刷新为新解析结果；同时重写了 `ISS-048` 那条回归测试（原来断言 `recipient_config` 重连后不变，现在断言的是"变了，变成新解析结果"）和 `test_wecom_api.py` 里 PUT 相关的集成用例（原来发 `recipient_config`，现在发 `field_mapping`）。后端 `ruff check`/`mypy` strict（150 个源文件）/`pytest`（wecom 相关全量 + 全量套件）均实际执行并通过，唯一失败仍是同源 `.env` 环境问题。前端 `lint`（含一次 prettier 自动修复）/`typecheck`/`test`（27 文件 261 项）/`build` 均通过；该组件历来没有专门的 Vitest 单测（同既有边界，未新增）。
- **未决**：这次修复没有专门再要一次真实抓包验证，逻辑是在 `ISS-052` 已有证据基础上做的产品层收紧，还需要用户下一次真实连接/同步确认收件人确实变成了模板审批人、且 -1000888 不再出现。

## 31. entrys 字段缺失时解析被误判为协议违规（`ISS-055`）

- 用户重新连接后，先报告执行同步阶段新提示"企业微信拒绝了本次请求(业务码 -5012)"。读原始日志：这是 `formcol/detail`（执行同步时的结构复核步骤，不是提交），响应体完全为空 `"body":{}`、`head.msg` 也是空的，跟之前几次不同——之前被拒时企业微信至少还会带一堆结构数据回来。用连接用的 `form_id` 前缀一查，跟用户之前发的 `获取reportvids.txt` 分析文件里那个表单完全一致，用问答确认：用户重连时用的正是那份文件对应的链接，而不是他平时提交日报实际用的那个——设置页粘贴框此前一直没被清空过（只有连接*成功*才会清空，而这个链接对应的表单此前没有一次真正连上过）。
- 用户随后又重新扫码登录了一次，这次错误变成连接阶段（`journal/get_template_combine_info`）报 `WeComProtocolChanged`、提示"企业微信返回内容不符合已知协议"。读原始日志发现这次 `head.ret` 其实是 `0`（企业微信没有拒绝），真正原因是响应体里根本没有 `entrys` 这个 key——这个账号从来没提交过这个表单，没有历史记录可列，企业微信干脆省略这个字段，而不是给一个空数组。`_parse_template_info()` 一直把 `entrys` 当必须存在的字段，缺失就直接判"协议不符"，报错信息完全没提到具体缺了什么，用户看到的只是一句通用提示。这是一个和收件人无关、纯粹的解析缺陷——过去所有测试账号连接的表单碰巧都已有提交历史，从没真实触发过这条路径。
- 修复：`entrys = body.get("entrys")` 改成 `body.get("entrys", [])`——key 整体缺失时按空列表处理，跟 `fork_items`/`appro` 同一套"确实缺失可以容忍、存在但形状不对仍然拒绝"的原则；key 存在但类型错误（比如给了个字符串）依然会被 `WeComProtocolChanged` 拒绝，不放松这一半的校验。新增 2 项 pytest 分别覆盖这两种情况。后端 `ruff check`/`mypy` strict（150 个源文件）/`pytest`（wecom 相关 + 全量）均通过，唯一失败仍是同源 `.env` 环境问题。
- **这个解析修复本身独立、已验证有效，但没有解决用户真正的问题**：即使解析不再报错，继续连接这个陌生表单也只是"技术上能连上"（会退回用 `template_info.appro` 当收件人兜底），并不代表这就是用户应该拿来同步的日报表单。已经提醒用户回到企业微信里平时实际填日报的入口重新拿链接、再连接一次，而不是继续用分析文件里那份链接。

## 32. PROJECT_LIST 新增类别和权重（CR-20260812-01）

- 后端 `ProjectListEntry` 新增 `category="重要"` 和 `weight=""`，严格保留 `project`/`content` 为唯一必填字段；旧日报、旧日期归档快照和旧周报内容在读取时自动补齐新字段，无数据库迁移。
- Electron/Vue 编辑器按“类别、工作项目、工作步骤、权重、预计完成时间节点、实际完成时间、责任人、协助人、所需资源支持、实际完成情况及解决措施”展示；未触碰的默认类别行保存前仍会过滤，用户修改类别或权重后则保留并由后端提示必填项目/步骤。
