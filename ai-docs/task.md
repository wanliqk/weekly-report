...
| ID | 主责 | 任务 | 依赖 | 状态 | 交付物与验收 |
|---|---|---|---|---|---|
| REQ-10 | Agent C | 整理同日多条目、日期级归档、管理员撤销、双账号初始化、菜单与统计需求 | 用户新需求 | REVIEW | `docs/需求理解.md` 第二版增量、`requirements.md` 覆盖规则、Given-When-Then 验收和待确认口径已完成，等待用户确认 |
| DESIGN-10 | 主 Agent | 第二版方案设计与迁移评审 | REQ-10、ISS-018..ISS-022 | TODO | 更新 `docs/方案设计.md`、architecture/database/api/modules/decisions；明确新表/约束、聚合事务、审计、权限、统计查询、V1 数据迁移与回滚 |
| BE-10 | Agent B | 第二版后端与 Alembic 实现 | DESIGN-10 | TODO | 双账号初始化、同日多条目、删除草稿、管理员撤销、日期级归档、周报/导出新来源、用户安全删除和个人统计 API |
| FE-10 | Agent A | 第二版 Electron/Vue 界面实现 | DESIGN-10、BE-10 契约 | TODO | 菜单改名、我的日报月历/条目列表/归档、我的周报适配、设置收口、管理员操作入口、用户删除和统计页 |
| QA-10 | Agent C | 第二版迁移、权限、并发、统计与 E2E 验收 | BE-10、FE-10 | TODO | 旧库迁移、同日并发、汇总原子性、权限隔离、删除保护、跨月/闰日统计及完整桌面流程通过门禁 |

第二版尚未进入方案设计或代码实现。REQ-10 未经用户确认、ISS-018..ISS-022 未形成设计结论前，不得抢跑 Alembic、API 或页面改造。
...
第二版当前仅 `REQ-10` 处于 `REVIEW`，等待用户确认需求口径；没有可领取的实现任务。确认后先执行 `DESIGN-10`，不得直接领取 `BE-10`/`FE-10`。
