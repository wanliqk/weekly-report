...
- 2026-08-11 完成 `ISS-054`（`PROD-032`）：用户没等确认 -1000888 是否解决，直接提出两条更根本的产品要求——"1. 永远不让用户手动去填vid。2. 系统必须指定模板里配置的审批人vid"，比 `ISS-052` 范围更大（要求彻底取消手动编辑能力，且首次连接无提交历史时也必须能自动拿到收件人）。新增 `WeComTemplateInfo.appro`（解析 `body.template_info.appro[]`，模板自身配置的审批人，不依赖提交历史，防御式解析）；收件人解析改为三级：`entries[0].reportvids` > `template_info.appro[].vid` > `wecom_vid` 自身兜底。`PUT /api/v1/wecom/profile` 移除 `recipient_config` 输入（`field_mapping` 改必填、唯一可更新项），`_upsert_profile()` 不再对 `recipient_config` 做"已存在就保留"，改成每次连接/重连都整体刷新（`PROD-030` 收窄为只管 `field_mapping`，`recipient_config` 分给 `PROD-032`）。`WeComSettingsCard.vue` 删掉两个手填 vid 多选框，改为只读展示。`docs/需求理解.md`/`docs/方案设计.md`（§5.1/§6.3/§9.1）/`ai-docs/api.md` 同步修订。新增 4 项后端 pytest + 重写 `ISS-048` 回归测试和 `test_wecom_api.py` 集成用例；后端 `ruff check`/`mypy` strict（150 个源文件）/`pytest` 全量通过（唯一失败同源 `.env` 问题），前端 `lint`/`typecheck`/`test`（261 项）/`build` 均通过。**这次没有专门再要真实抓包验证，是在 `ISS-052` 证据基础上做的产品层收紧，还需要用户下一次真实连接/同步确认收件人变成模板审批人、-1000888 不再出现。**
- 2026-08-11 完成 `ISS-055`：用户重连后先报 `formcol/detail` 被拒（`business_code=-5012`，响应体完全为空），排查发现用户重连时用的是之前分析用的 `获取reportvids.txt` 抓包对应的表单链接、不是他日常提交日报的表单（设置页粘贴框未清空，因为此前该链接从没连接成功过）。用户又重新扫码一次，这次是连接阶段报 `WeComProtocolChanged`（"响应内容不符合已知协议"），读原始日志发现 `head.ret=0`（企业微信没拒绝），真正原因是响应缺少 `entrys` key（该账号从未提交过这个表单、无历史可列，企业微信直接省略该字段而非给空数组）——`_parse_template_info()` 一直把 `entrys` 当必需字段，是一个和收件人无关、过去测试账号恰好都有提交历史而从未真实触发过的解析缺陷。修复：`body.get("entrys", [])`，key 缺失按空列表容忍、类型错误仍拒绝，同 `fork_items`/`appro` 已有原则一致。新增 2 项 pytest，后端 `ruff check`/`mypy` strict（150 个源文件）/`pytest` 全量通过（唯一失败同源 `.env` 问题）。**这个解析修复独立有效，但没解决用户真正的问题**：已提醒用户去企业微信里平时实际填日报的入口重新拿链接连接，而不是继续用分析文件里那份表单——即使解析不报错了，也不代表这就是该同步的表单。
- 2026-08-11 完成 `ISS-056`：用户改用日常实际填报的表单重连后，同步仍失败于同一提示"企业微信拒绝了本次请求(业务码 -5012)"——这次不是表单选错。核对 `wecom.log`/`wecom-raw-debug.log` 确认执行同步阶段调用的 `formcol/detail`（`get_form_detail()`）对该真实表单/fork 持续返回空 `body:{}` 和 `business_code=-5012`，4 次独立尝试跨约 17 小时全部复现，而同一时间段内同一表单/fork 上的 `journal/get_template_combine_info`（连接阶段沿用）均成功——两个接口对完全相同目标出现结构性不对称拒绝，无法在 Client 一侧规避。用户提供真实成功抓包，证实 `GET formcol/answer_page?_prefetch=1`（与 `submit_daily()` 已在使用的 POST 同一路径，仅换成 GET 并加 `_prefetch=1`）能成功返回完整结构（`body.form.{form_id,create_vid,create_name,question.items[]}`，字段名与 `get_template_combine_info` 完全一致，可复用同一解析）。`get_form_detail()` 已改用该接口取代 `formcol/detail`；`WeComFormDetail.fork_items`/`WeComFormDetailForkItem`/`_resolve_current_fork_form_id()`/`_MAX_FORK_ITEMS` 全部删除——新接口不返回任何 fork/提交历史字段，`ISS-051` 的"提交前选最新 fork"机制因此彻底失去数据来源。**这是对 `ISS-051` 所防护问题（长期不重连导致 fork 漂移、提交目标过期被拒）的一次刻意、已知的回退，不是静默丢弃**：提交固定退回为直接使用 `wecom_sync_profiles.form_id`，与 `ISS-051` 之前的行为一致；已记录建议长期使用中定期重新连接企业微信以缓解 fork 漂移风险。`docs/方案设计.md` §2.4/§5.3/§6.3/§10.2/§10.3 已追加 2026-08-11 修订说明（§10.2 第四次修订）。测试全面重写：`test_wecom_client.py` 用新合成 fixture `formcol_answer_page_get_response.json`（替换已删除的 `formcol_detail_response.json`）覆盖新响应形状与题目结构校验，`test_wecom_logging.py`/`test_wecom_mapper.py`/`wecom_service_support.py`/`test_wecom_sync_service.py`/`test_wecom_internal_api.py` 移除全部 fork 相关测试/辅助函数，改为单一"提交固定使用 `profile.form_id`"回归测试。后端 `ruff check`/`ruff format --check`（仅剩已知无关的 `ISS-029`）/`mypy` strict（150 个源文件）/`pytest`（全量 502 项，501 passed + 1 failed，唯一失败为同源 `.env` 预置问题）均已实际执行通过。**尚未经用户真实重试确认 -5012 消失**，下一个新会话如果用户报告同一或新业务码，需要重新评估。
