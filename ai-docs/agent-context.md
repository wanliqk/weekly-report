...
- 当前阶段：CR-20260808-02 已完成 `WECOM-00`（敏感样例治理）、`WECOM-01`（文档）、`WECOM-02`（数据基础）、`WECOM-03`（Electron 登录与凭证桥）；企业微信协议 Client、字段映射、同步 Service、API、renderer UI 仍未实现，`wecom_sync` 占位保持有效。
- 下一步：`WECOM-04`（内部协议 Client，依赖 `WECOM-00`/`WECOM-02`/`WECOM-03` 均已满足）与 `WECOM-05`（字段映射与预览，依赖 `WECOM-02` 已满足）可并行领取，二者文件范围不重叠；`WECOM-06` 需等待两者都完成。测试一律使用 `backend/tests/fixtures/wecom/` 的合成 fixture，不得读取或依赖已被 `.gitignore` 排除的 `backend/wx-ribao/`。
- 未获用户明确授权，不得推送远端。
