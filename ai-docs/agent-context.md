...
- 阶段 6 导出任务/归档校验/动态列规划、xlsx 生成/下载/过期清理、Electron 保存对话框白名单和日报导出交互已完成实现、自测、质量门禁、独立审查（含专项安全审查）与提交，五项任务统一为 `DONE`；周报、设置/能力占位 UI、手动备份和发布产物（PyInstaller/安装包）尚未实现。
...
- 已完成阶段：工程基线（`3a9fdbc`）、Desktop Bootstrap（`7386cae`）、数据基础与 API Foundation（`8480515`）、认证与用户管理（`1a50e75`）、模板、设置与日报闭环（`1d965fe`）、查询导出与桌面保存（`d6ab86e`）。
- 当前阶段：阶段 6 已完成；阶段 7 周报闭环尚未开始。
- 下一步：进入阶段 7，先复核自然周可用性、生成和来源快照契约（`WEEKLY-01`），再按依赖推进编辑/乐观锁/重生成确认和前端交互。
- 阶段 7 完成前，不得把设置/能力占位 UI、手动备份或发布能力标为已完成；未获授权不得推送远端。
