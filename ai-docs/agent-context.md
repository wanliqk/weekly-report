...
- 阶段 5 模板发布、个人设置/能力 API、日报快照与状态机及对应 Vue 页面已完成实现、自测、质量门禁、独立审查和提交 `1d965fe`，七项任务统一为 `DONE`；阶段 6 已从 `EXPORT-01` 开始，周报、设置/能力占位 UI、手动备份和发布产物（PyInstaller/安装包）尚未实现。
...
- 已完成阶段：工程基线（`3a9fdbc`）、Desktop Bootstrap（`7386cae`）、数据基础与 API Foundation（`8480515`）、认证与用户管理（`1a50e75`）、模板、设置与日报闭环（`1d965fe`）。
- 当前阶段：阶段 6 查询导出与桌面保存；`EXPORT-01` 为 `IN_PROGRESS`，其余阶段任务为 `TODO`。
- 下一步：复核冻结的导出/保存契约，实现导出任务、归档校验与动态列规划，再按依赖推进 xlsx、桌面保存和前端交互。
- 阶段 6 完成前，不得把周报、手动备份或发布能力标为已完成；未获授权不得推送远端。
