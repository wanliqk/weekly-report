...
资料：最初的脚本 `wx-ribao.py` 与一次真实提交的原始 HTTP 抓包（`http_raw_request.txt`/
`http_raw_response.txt`），以及 2026-08-09 补充的 `ribao.txt`（`GET formcol/detail` → `POST
formcol/answer_page` 的真实成功抓包，`ISS-040`）。本目录的 fixture 参考了那份真实资料**归纳出的
协议形状**（字段名、嵌套结构、成功判定字段），但不包含其中任何真实取值；两者不应被混淆，也不允许
把真实文件里的任何值复制粘贴进本目录。
...
- 新增或修改本目录文件时，只允许写入合成数据；`backend/tests/test_wecom_fixture_hygiene.py`
  会在本机存在 `backend/wx-ribao/` 时，动态提取真实样例（含 `ribao.txt`）里的 token 并断言本目录
  不包含，作为防止手滑粘贴真实值的回归防线。
